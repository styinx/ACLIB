...
from settings import STYLE_DIR, CONFIG_DIR
...
class WidgetConfig:
    ...
    def load_app_config(app: object, app_name: str):
        app_config_file = os.path.join(CONFIG_DIR, app_name + '.ini')
...
class WidgetStyle:
    ...
    def load_style_from_config(obj: object, class_name: str, app_name: str = None):
        ...
        if class_name not in WidgetStyle.STYLE:
            ...
            if app_name:
                app_style_file = os.path.join(STYLE_DIR, app_name + '.ini')
                if os.path.isfile(app_style_file):
                    config = Config(app_style_file)

            # Try to load from widget style config
            if not config:
                class_style_file = os.path.join(STYLE_DIR, class_name + '.ini')
